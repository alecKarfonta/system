...
# Legacy archive root (manual_deployments/)
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LEGACY="$(cd "$SCRIPT_DIR/.." && pwd)"
cd "$LEGACY"

# Check if legacy archive is intact
if [[ ! -f "README.md" ]] || [[ ! -d "setup" ]]; then
    print_error "Legacy archive not found. Expected manual_deployments/ layout."
    exit 1
fi

echo -e "${YELLOW}Note: This is a legacy setup script. Active development uses k3s (see repo root).${NC}\n"
